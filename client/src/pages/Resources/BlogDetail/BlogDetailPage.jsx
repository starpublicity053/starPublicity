...
import {
  useGetBlogByIdQuery,
  useGetBlogsQuery,
  useSubmitContactFormMutation, // Import the new mutation hook
...
const generateContentHTML = (blocks) => {
  ...
  while (i < blocks.length) {
    ...
    // Check for consecutive image blocks to render them in a row
    if (
      block.type === "image" &&
      i + 1 < blocks.length &&
      ...
      `;
      i += 2; // Skip the next image as it's already processed
      continue;
    }

    switch (block.type) {
      case "heading":
        // Define specific classes for each heading level for more direct styling.
      ...
      case "image":
        ...
        }" class="rounded-2xl shadow-lg w-full h-auto object-cover mx-auto" />
            ${
              block.caption
...
const calculateReadTime = (blocks) => {
  if (!Array.isArray(blocks)) {
    return 1; // Default to 1 minute if content is not an array
  }
...
const BlogDetailPage = () => {
  ...
  ] = useSubmitContactFormMutation(); // Initialize the mutation hook
  ...
  // Function to handle tag click and navigate to blogs page with filter
  const handleTagClick = (tag) => {
    const encodedTag = encodeURIComponent(tag);
    navigate(`/resources/blogs?tag=${encodedTag}`);
  };
  ...
  // Safely parse the blog content from a JSON string into an array.
  let parsedContentBlocks = [];
  if (post && post.content) {
    try {
      if (typeof post.content === "string") {
        parsedContentBlocks = JSON.parse(post.content);
      } else if (Array.isArray(post.content)) {
        parsedContentBlocks = post.content;
      } else {
        parsedContentBlocks = [
  ...
  return (
    <div className="min-h-screen bg-white font-sans text-gray-800">
        ...
        <div className="absolute inset-x-0 bottom-0 z-10 px-4 sm:px-6 lg:px-8 py-8 md:py-12 text-white animate-slide-in-down">
          <div className="max-w-7xl mx-auto">
            <div className="text-left">
              <h1 className="text-5xl sm:text-6xl md:text-7xl lg:text-8xl font-bold leading-tight tracking-tight drop-shadow-lg mb-6">
                {post.title}
              </h1>
              <div className="flex flex-wrap items-center gap-y-3 gap-x-6 text-gray-200 text-xl md:text-2xl">
                <div className="flex items-center space-x-3">
                  <FaUserCircle className="w-10 h-10 text-[#7A85C1]" />
                  <span className="font-semibold">{post.author}</span>
                </div>
                <div className="flex items-center space-x-2">
                  <FaCalendarAlt className="text-xl text-[#7A85C1]" />
                  <span>{formattedDate}</span>
                </div>
                <div className="flex items-center space-x-2">
                  <FaClock className="text-xl text-[#7A85C1]" />
                  <span>{readTime} min read</span>
                </div>
              </div>
              {/* REMOVED TAGS FROM HERE */}
            </div>
          </div>
        </div>
      </header>
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 pb-16">
        <div className="lg:grid lg:grid-cols-3 lg:gap-12" ref={contentRef}>
          <main className="lg:col-span-2 mb-12 lg:mb-0">
            <article
              className="prose prose-lg max-w-3xl mx-auto lg:mx-0
                              prose-p:leading-relaxed prose-p:mb-5
                              prose-a:text-blue-600 prose-a:hover:text-blue-800 prose-a:font-medium prose-a:underline
                              prose-blockquote:border-l-4 prose-blockquote:border-blue-500 prose-blockquote:pl-4 prose-blockquote:italic prose-blockquote:text-blue-800 prose-blockquote:bg-blue-50 prose-blockquote:py-3 prose-blockquote:px-5 prose-blockquote:rounded-r-md prose-blockquote:shadow-sm
                              prose-li:mb-2 prose-ul:list-disc prose-ul:pl-5 prose-ul:marker:text-blue-500
                              prose-ol:list-decimal prose-ol:pl-5 prose-ol:marker:text-blue-500
                              prose-img:rounded-2xl prose-img:shadow-lg prose-img:border prose-img:border-gray-200
                              focus:outline-none focus:ring-2 focus:ring-[#1A2A80]focus:ring-opacity-50"
            >
                  ...
                  <ul className="list-none space-y-2 pl-0">
                    {post.keyHighlights.map((highlight, index) => (
                      <li key={index} className="flex items-start">
                        <span className="text-[#1A2A80] font-bold mr-2">
                          &#10003;
                        </span>
                        <span>{highlight}</span>
                      </li>
            ...
            <section className="mt-16 bg-blue-50 p-8 rounded-xl shadow-lg border border-blue-100 text-center">
              <h3 className="text-3xl font-bold text-[#1A2A80] mb-6 leading-tight">
                Ready to Boost Your Brand with OOH?
              </h3>
              <p className="text-lg text-gray-700 mb-10 max-w-2xl mx-auto">
                Discover how our strategic insights and innovative solutions can
                elevate your next outdoor advertising campaign.
              </p>
              <a
                href="/contact"
                className="inline-flex items-center justify-center px-8 py-4 bg-[#1A2A80] text-white font-bold rounded-full shadow-lg hover:bg-blue-700 transition duration-300 transform hover:scale-105 text-lg"
              >
          ...
          <aside className="lg:col-span-1 hidden lg:block">
            <div
              id="contact-form"
              className="relative p-8 rounded-2xl shadow-xl border border-blue-200 mb-8 sticky top-8 overflow-hidden bg-white dark:bg-gray-800 before:absolute before:inset-0 before:bg-gradient-to-br before:from-blue-50 before:to-indigo-100 before:opacity-80 dark:before:from-gray-900 dark:before:to-blue-950 before:z-0"
            >
              ...
              <div className="relative z-20 text-center mb-8">
                <IoMailOpenOutline className="text-[#1A2A80] dark:text-blue-400 text-6xl mx-auto mb-4 animate-bounce-subtle" />
                <h3 className="text-3xl font-extrabold text-[#1A2A80] dark:text-white drop-shadow-sm leading-tight">
                  Let's Connect!
                </h3>
              ...
              <form onSubmit={handleSubmit} className="relative z-20 space-y-6">
                ...
                <button
                  type="submit"
                  disabled={isSubmitting || isSubmitSuccess} // <-- Add isSubmitSuccess
                ...
                >
                  <span className="absolute inset-0 bg-[#1A2A80] transform scale-x-0 group-hover:scale-x-100 transition-transform duration-300 origin-left"></span>
                    ...
                    <li key={relatedPost._id}>
                      ...
                      >
                        <img
                          ...
                          className="w-20 h-20 object-cover rounded-md mr-4 flex-shrink-0 border border-gray-200 group-hover:scale-105 transition-transform duration-200"
                        />
                        <span className="text-lg text-gray-800 font-medium group-hover:text-[#1A2A80] leading-tight">
                          {relatedPost.title}
                        </span>
                      </Link>
                    </li>
                ...
                {post.tags &&
                  post.tags.map((tag, index) => (
                    <button
                      ...
                      className="px-4 py-2 bg-blue-100 text-[#1A2A80] text-base font-medium rounded-full hover:bg-blue-200 hover:text-blue-800 transition-colors duration-200 shadow-sm cursor-pointer"
                    >
                      {tag}
                    </button>
                  ))}
      ...
      <div
        className={`fixed bottom-8 right-8 z-40 bg-white p-4 rounded-full shadow-xl transition-all duration-300 ${
          showStickyShare
            ? "opacity-100 scale-100"
            : "opacity-0 scale-90 pointer-events-none"
        } hidden md:flex flex-col space-y-4 border border-blue-100`}
      >
        <a
