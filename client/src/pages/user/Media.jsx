...
const RequestCallbackModal = ({ isOpen, onClose }) => {
    ...
    const handleSubmit = (e) => { e.preventDefault(); console.log("Callback Request Submitted:", formData); alert(`Thank you, ${formData.name}! We will call you back shortly.`); onClose(); };
        ...
        <AnimatePresence>
            {isOpen && (
                <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} onClick={onClose} className="fixed inset-0 bg-black/60 backdrop-blur-sm z-50 flex items-center justify-center p-4">
                    <motion.div initial={{ scale: 0.9, opacity: 0, y: 20 }} animate={{ scale: 1, opacity: 1, y: 0 }} exit={{ scale: 0.9, opacity: 0, y: 20 }} transition={{ type: "spring", damping: 20, stiffness: 200 }} onClick={(e) => e.stopPropagation()} className="relative w-full max-w-md bg-white rounded-2xl shadow-2xl p-6 md:p-8">
                        ...
                        <form onSubmit={handleSubmit} className="space-y-4">
                            <div className="relative"><User className="absolute left-3 top-1/2 -translate-y-1/2 text-slate-400" size={20} /><input type="text" name="name" placeholder="Full Name" value={formData.name} onChange={handleChange} required className="w-full pl-10 pr-4 py-3 border border-slate-300 rounded-lg focus:ring-2 focus:ring-[#1a2a80] focus:border-transparent outline-none transition" /></div>
                            <div className="relative"><Phone className="absolute left-3 top-1/2 -translate-y-1/2 text-slate-400" size={20} /><input type="tel" name="phone" placeholder="Phone Number" value={formData.phone} onChange={handleChange} required className="w-full pl-10 pr-4 py-3 border border-slate-300 rounded-lg focus:ring-2 focus:ring-[#1a2a80] focus:border-transparent outline-none transition" /></div>
                            <div className="relative"><Briefcase className="absolute left-3 top-1/2 -translate-y-1/2 text-slate-400" size={20} /><input type="text" name="company" placeholder="Company Name (Optional)" value={formData.company} onChange={handleChange} className="w-full pl-10 pr-4 py-3 border border-slate-300 rounded-lg focus:ring-2 focus:ring-[#1a2a80] focus:border-transparent outline-none transition" /></div>
                            <button type="submit" className="w-full flex items-center justify-center gap-2 bg-[#1a2a80] text-white font-bold py-3 px-4 rounded-lg transition-transform hover:scale-105 shadow-lg shadow-indigo-900/20 text-lg mt-4"><Send size={20} />Submit Request</button>
                        </form>
                    </motion.div>
                </motion.div>
            )}
...
const ModernHeroSection = ({ onOpenModal }) => {
    ...
    return (
        // RESPONSIVE: Added responsive padding for better spacing on all screens
        <section className="relative w-full min-h-screen flex items-center justify-center overflow-hidden px-4 sm:px-6 lg:px-8 py-24">
            ...
            {/* RESPONSIVE: Added adaptive max-width to the container */}
            <div className="relative z-10 w-full max-w-xl md:max-w-3xl lg:max-w-5xl mx-auto flex flex-col items-center text-center">
                <motion.h1
                    ...
                    // RESPONSIVE: Smoother font size scaling across breakpoints
                ...
                >
                    ...
                    <motion.span variants={{ hidden: { opacity: 0, y: 20 }, visible: { opacity: 1, y: 0, transition: { type: 'spring', stiffness: 100 } } }} className="brand-color-text block mt-1 md:mt-2">Unmissable Results.</motion.span>
                </motion.h1>
                ...
                <motion.p 
                    ...
                    // RESPONSIVE: Adjusted text size for readability
                ...
                <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.7, ease: 'easeOut', delay: 0.6 }} className="mt-12 w-full grid grid-cols-1 md:grid-cols-3 gap-6">
                    <StatCard icon={<MapPin size={28} />} value="40+" label="Cities Covered" description="Our network spans major commercial and transit hubs, putting your brand at the heart of the action where it matters most." />
                    <StatCard icon={<ShoppingBag size={28} />} value="500+" label="Brands Served" description="From national industry leaders to cherished local favourites, brands trust us to deliver their message with impact and precision." />
                    <StatCard icon={<BarChart size={28} />} value="95M+" label="Monthly Impressions" description="Generate massive brand awareness with strategically placed media that guarantees millions of views each month." />
                </motion.div>
                ...
                <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.7, ease: 'easeOut', delay: 0.8 }} className="mt-12 flex flex-col sm:flex-row gap-4 justify-center">
                    <motion.a href="#media-finder-section" onClick={handleScrollClick} whileHover={{ scale: 1.05, y: -2 }} whileTap={{ scale: 0.95 }} className="flex items-center justify-center gap-3 bg-[#1a2a80] text-white font-bold py-3 px-6 md:py-4 md:px-8 rounded-lg shadow-lg shadow-indigo-900/30 text-base md:text-lg">
                        <Rocket size={20} /> Start Your Campaign
                    </motion.a>
                    <motion.button onClick={onOpenModal} whileHover={{ scale: 1.05, y: -2 }} whileTap={{ scale: 0.95 }} className="flex items-center justify-center gap-3 bg-white text-slate-800 font-bold py-3 px-6 md:py-4 md:px-8 rounded-lg border-2 border-slate-300 text-base md:text-lg">
...
// --- Media Finder Section (Already responsive, minor tweaks for consistency) ---
        ...
        <section id="media-finder-section" className="w-full bg-gray-50 text-black py-16 sm:py-20 px-4 sm:px-6 lg:px-8"> 
            <div className="max-w-screen-2xl mx-auto"> 
                <div className="text-center mb-12 sm:mb-16"> 
                    ...
                    <p className="text-base sm:text-lg text-gray-600 max-w-3xl mx-auto">Filter by state and city to explore our real-time inventory of billboards, transit ads, and more. Your ideal campaign launchpad is just a few clicks away.</p> 
                </div> 
            ...
            <div className="bg-white">
                ...
                <div className="max-w-screen-xl mx-auto w-full">
                    <div className="max-w-4xl mx-auto text-center py-16 px-4 sm:px-6 lg:px-8">
                        ...
                        <p className="text-base md:text-xl mb-0 max-w-3xl mx-auto text-gray-600">From city center hotspots to vibrant corners, our diverse outdoor media connects your brand with audiences wherever they move.</p>
                    </div>
