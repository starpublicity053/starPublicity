...
// Custom component for the official X logo SVG
...
const animationStyles = `
    ...
    .pulsing-marker { background-color: #1a2a80; width: 16px; height: 16px; border-radius: 50%; box-shadow: 0 0 10px #1a2a80, 0 0 20px #1a2a80; animation: pulse-blue 2s infinite; }
    @keyframes pulse-blue { 0% { transform: scale(0.95); box-shadow: 0 0 0 0 rgba(26, 42, 128, 0.7); } 70% { transform: scale(1); box-shadow: 0 0 10px 10px rgba(26, 42, 128, 0); } 100% { transform: scale(0.95); box-shadow: 0 0 0 0 rgba(26, 42, 128, 0); } }
    .pulsing-marker-yellow { box-shadow: 0 0 8px #facc15, 0 0 16px #facc15; animation: pulse-yellow 2s infinite; }
    @keyframes pulse-yellow { 0% { transform: scale(0.95); box-shadow: 0 0 0 0 rgba(250, 204, 21, 0.7); } 70% { transform: scale(1); box-shadow: 0 0 8px 8px rgba(250, 204, 21, 0); } 100% { transform: scale(0.95); box-shadow: 0 0 0 0 rgba(250, 204, 21, 0); } }
`;
  ...
  return (
    <>
      ...
      <div className="w-full bg-gray-100 font-poppins text-gray-800 relative overflow-hidden">
        {/* === Hero Section === */}
        {/* NAVBAR FIX: Increased top padding (pt-24) to prevent overlap */}
          ...
          <motion.div variants={containerVariants} initial="hidden" animate="visible" className="w-full max-w-7xl mx-auto grid grid-cols-1 lg:grid-cols-2 gap-10 items-center">
            <div className="flex flex-col text-center lg:text-left">
              <motion.div ref={headingRef} onMouseMove={handleMouseMove} onMouseLeave={handleMouseLeave} className="flex flex-col items-center lg:items-start">
                <motion.h2 initial={{ opacity: 0, y: 40 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.8, ease: "easeOut", delay: 0.2 }} className="text-2xl sm:text-4xl lg:text-5xl font-semibold text-gray-600 tracking-tight">
                  Let's Make Your
                </motion.h2>
                <div style={{ perspective: "1000px" }}>
                  <motion.h1 style={{ rotateX, rotateY, transformStyle: "preserve-3d" }} initial={{ opacity: 0, scale: 0.8 }} animate={{ opacity: 1, scale: 1 }} transition={{ duration: 0.6, ease: [0.6, -0.05, 0.01, 0.99], delay: 0.4 }} className="text-5xl sm:text-7xl md:text-8xl lg:text-9xl font-black text-[#1a2a80] tracking-tighter my-1">
                    BRAND
                  </motion.h1>
                </div>
                <motion.h2 initial={{ opacity: 0, y: 40 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.8, ease: "easeOut", delay: 0.6 }} className="text-2xl sm:text-4xl lg:text-5xl font-semibold text-gray-600 tracking-tight self-center lg:self-end">
                  Unforgettable.
                </motion.h2>
              </motion.div>
              <motion.p variants={itemVariants} className="mt-6 text-base md:text-lg text-gray-600 max-w-lg mx-auto lg:mx-0">
              ...
              <motion.div variants={itemVariants} className="mt-8 flex flex-col sm:flex-row items-center justify-center lg:justify-start gap-x-8 gap-y-3 text-gray-500">
                <div>
                  <Magnetic>
                    <a href="mailto:info@starpublicity.co.in" className="block hover:text-[#1a2a80] transition-colors">
                ...
                <div>
                  <Magnetic>
                    <a href="tel:01614668602" className="block hover:text-[#1a2a80] transition-colors">
                      0161-4668602
                    </a>
                  </Magnetic>
                </div>
            ...
            <motion.div variants={itemVariants} className="bg-white/50 backdrop-blur-sm p-8 lg:p-10 rounded-2xl shadow-lg border border-white/50">
              <motion.form id="contact-form" className="space-y-6 text-left" onSubmit={handleSubmit}>
        ...
        <div ref={mapSectionRef} className="w-full relative flex flex-col items-center justify-center overflow-hidden py-16 sm:py-20 lg:py-24 px-4 sm:px-8 bg-gray-50 bg-[radial-gradient(ellipse_at_center,_var(--tw-gradient-stops))] from-white via-gray-100 to-gray-200">
          ...
          <div className="relative z-10 w-full max-w-7xl mx-auto flex flex-col items-center text-center px-4">
            ...
            <motion.p initial={{ opacity: 0, y: 20 }} animate={mapInView ? { opacity: 1, y: 0 } : {}} transition={{ duration: 0.6, delay: 0.2 }} className="text-gray-600 text-base md:text-lg max-w-3xl mx-auto mb-10 sm:mb-12">
              From our strategic hub in Ludhiana, we combine geographical dominance with creative technology to put your brand on the map and in the minds of millions.
            </motion.p>
          </div>

          <div className="relative z-10 w-full max-w-7xl mx-auto flex justify-center items-center">
            <motion.div variants={containerVariants} initial="hidden" animate={mapInView ? "visible" : "hidden"} className="relative z-10 flex flex-col items-center w-full">
              <motion.div variants={itemVariants} className="relative w-11/12 aspect-square max-w-[400px] sm:w-full sm:max-w-[500px] flex justify-center items-center">
                ...
                <div className="absolute w-full h-full animate-[orbit-outer_60s_ease-in-out_infinite]">
                  {outerNodes.map((node, index) => {
                    ...
                    return (
                      <div key={`outer-${index}`} className="absolute inset-0 pointer-events-none" style={{ transform: `rotate(${node.angle}deg)` }}>
                        <Component {...linkProps} aria-label={node.label} className="absolute top-0 left-1/2 -translate-x-1/2 cursor-pointer group icon-wrapper-outer pointer-events-auto">
                          <div className="relative flex items-center justify-center w-12 h-12 sm:w-14 sm:h-14">
                            ...
                            <div className="relative flex items-center justify-center w-10 h-10 sm:w-12 sm:h-12 bg-white/80 border border-purple-400/50 rounded-full backdrop-blur-sm transition-all group-hover:scale-105">
                              <node.icon size={20} className="text-primary-blue" />
                            </div>
                          </div>
                        </Component>
                      </div>
                ...
                <div className="absolute w-[64.2%] h-[64.2%] animate-[orbit-inner_40s_ease-in-out_infinite]">
                  {innerNodes.map((node, index) => {
                    ...
                    const linkProps = { [isPathLink ? "to" : "href"]: node.href, ...(isExternal && { target: "_blank", rel: "noopener noreferrer" }) };
                    return (
                      <div key={`inner-${index}`} className="absolute inset-0 pointer-events-none" style={{ transform: `rotate(${node.angle}deg)` }}>
                        <Component {...linkProps} aria-label={node.label} className="absolute top-0 left-1/2 -translate-x-1/2 cursor-pointer group icon-wrapper-inner pointer-events-auto">
                          <div className="relative flex items-center justify-center w-10 h-10 sm:w-12 sm:h-12">
                            ...
                            <div className="relative flex items-center justify-center w-8 h-8 sm:w-10 sm:h-10 bg-white/80 border border-purple-400/50 rounded-full backdrop-blur-sm transition-all group-hover:scale-105">
                              <node.icon size={16} className="text-primary-blue" />
                            </div>
                          </div>
                        </Component>
                      </div>
                ...
                <div className="relative flex justify-center items-center">
                  <motion.div initial={{ opacity: 0, scale: 0.8 }} animate={mapInView ? { opacity: 1, scale: 1 } : {}} transition={{ duration: 0.6 }} className="relative z-10 w-48 h-48 sm:w-64 sm:h-64 rounded-full overflow-hidden">
                    {animationData && (<Lottie animationData={animationData} loop={true} autoplay={true} className="w-full h-full" />)}
                  </motion.div>
                  
                  <motion.div animate={{ opacity: [0, 1, 1, 0], scale: [0.5, 1, 1, 0.5], y: [10, 0, 0, 10] }} transition={{ duration: 5, repeat: Infinity, repeatType: "loop", times: [0, 0.2, 0.8, 1] }} className="absolute -top-12 left-1/2 -translate-x-1/2 w-40 sm:left-auto sm:translate-x-0 sm:-top-8 sm:-right-28 sm:w-56 z-20">
                    <svg viewBox="0 0 200 130" className="drop-shadow-lg filter">
                      <path d="M171.3,5.1C148.9-6,117.2-1.3,100,10.1C82.8-1.3,51.1-6,28.7,5.1C-3.4,20.6-9.1,57.1,17.4,82.4c10.4,9.9,25.3,15,40.5,14.6c2.4,10,10.2,17.9,20.7,21.1c-0.8-4.7-1.3-9.5-1.5-14.3c-0.2-5.4,0-10.8,0.5-16.2c7.2,1.3,14.7,1.8,22.3,1.5c18-0.7,35-6.7,46.8-19.3C196.1,65.3,193.8,20.6,171.3,5.1z" fill="#FFFFFF" />
                    </svg>
        ...
        <div className="w-full flex flex-col justify-center items-center relative overflow-hidden font-poppins py-16 sm:py-20 px-4 sm:px-8 xl:px-16 z-10 bg-gray-50">
          <div className="relative z-10 text-center flex flex-col items-center w-full max-w-6xl mx-auto">
            ...
            <p className="text-gray-600 text-base md:text-lg max-w-2xl mx-auto mb-10 sm:mb-12">
              We combine speed, strategy, and creative excellence to deliver campaigns that get noticed.
            </p>
