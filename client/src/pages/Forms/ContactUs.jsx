...
const animationStyles = `
    ...
    /* These control the main rotation for icons and the new dashed trails */
    ...
    /* Utility class to set the rotation point for SVG elements */
    .rotate-from-center {
      transform-origin: center;
    }
    
    /* Unchanged supporting animations */
...
// Component for the promise feature cards
    ...
    return (
        <>
            ...
            <div className="min-h-screen flex flex-col items-center bg-gray-100 font-poppins text-gray-800">
                        ...
                        <div className="bg-[#1a2a80] text-white p-10 lg:p-16 flex flex-col justify-center">
                            ...
                            <div className="space-y-8">
                                <div className="flex items-center space-x-4">
                                    <div className="p-3 bg-white/10 rounded-xl"><Mail className="w-6 h-6 text-yellow-400" /></div>
                                    <div>
                                        <h3 className="font-semibold">Email</h3>
                                        <a href="mailto:hello@yourwebsite.com" className="text-gray-300 hover:text-yellow-400 transition">hello@yourwebsite.com</a>
                                    </div>
                                </div>
                                <div className="flex items-center space-x-4">
                                    <div className="p-3 bg-white/10 rounded-xl"><Phone className="w-6 h-6 text-yellow-400" /></div>
                                    <div>
                                        <h3 className="font-semibold">Phone</h3>
                                        <a href="tel:01614668602" className="text-gray-300 hover:text-yellow-400 transition">0161-4668602</a>
                                    </div>
                                </div>
                                <div className="flex items-center space-x-4">
                                    <div className="p-3 bg-white/10 rounded-xl"><MapPin className="w-6 h-6 text-yellow-400" /></div>
                                    <div>
                                        <h3 className="font-semibold">Location</h3>
                                        <p className="text-gray-300">Feroze gandhi market <br /> ludhiana, punjab</p>
                                    </div>
                                </div>
                            </div>
                        </div>
                            ...
                            <div className="w-full">
                                ...
                                <form className="space-y-6" onSubmit={handleSubmit}>
                                    <div>
                                        ...
                                        <input type="text" id="name" placeholder="Enter your name" value={formData.name} onChange={handleChange} className="w-full px-4 py-3 rounded-lg bg-white border border-gray-300 focus:ring-2 focus:ring-[#1a2a80] focus:border-[#1a2a80] shadow-sm outline-none" />
                                    </div>
                                    <div>
                                        <label htmlFor="email" className="block text-sm font-medium text-gray-700 mb-2">Email Address</label>
                                        <input type="email" id="email" placeholder="Enter your email" value={formData.email} onChange={handleChange} className="w-full px-4 py-3 rounded-lg bg-white border border-gray-300 focus:ring-2 focus:ring-[#1a2a80] focus:border-[#1a2a80] shadow-sm outline-none" />
                                    </div>
                                    <div>
                                        <label htmlFor="message" className="block text-sm font-medium text-gray-700 mb-2">Your Message</label>
                                        <textarea id="message" rows="4" placeholder="How can we help you?" value={formData.message} onChange={handleChange} className="w-full px-4 py-3 rounded-lg bg-white border border-gray-300 focus:ring-2 focus:ring-[#1a2a80] focus:border-[#1a2a80] shadow-sm outline-none" />
                                    </div>
                                    <motion.button whileHover={{ scale: 1.02 }} whileTap={{ scale: 0.98 }} type="submit" disabled={isLoading} className="w-full flex items-center justify-center space-x-2 bg-[#1a2a80] text-white font-semibold py-4 px-6 rounded-lg shadow-md hover:bg-[#152063] transition disabled:opacity-50">
                                        {isLoading ? 'Sending...' : <><Send className="w-5 h-5" /><span>Submit</span></>}
                                    </motion.button>
                                    {submitStatus.message && <p className={`mt-4 text-center ${submitStatus.type === 'success' ? 'text-green-500' : 'text-red-500'}`}>{submitStatus.message}</p>}
                                </form>
                            </div>
                        </div>
                    </motion.div>
                ...
                {/* === SECOND SECTION (REVISED & CORRECTED) === */}
                <div ref={sectionRef} className="min-h-screen w-full flex flex-col justify-center items-center relative overflow-hidden py-20 pb-40 z-10 bg-gray-50 bg-[radial-gradient(ellipse_at_center,_var(--tw-gradient-stops))] from-white via-gray-100 to-gray-200">
                    ...
                    <motion.div variants={containerVariants} initial="hidden" animate={inView ? "visible" : "hidden"} className="relative z-10 text-center flex flex-col items-center max-w-6xl mx-auto px-4">
                        <motion.div variants={itemVariants} className="mb-8 md:mb-12">
                            <h2 className="text-4xl md:text-5xl font-extrabold text-[#1a2a80] tracking-tight drop-shadow-lg leading-snug">Step Inside Our World</h2>
                            <p className="text-gray-600 mt-2 md:mt-4 text-lg max-w-xl mx-auto">Explore our services and take a virtual tour of our innovative space.</p>
                        </motion.div>
                        ...
                        <motion.div variants={itemVariants} className="relative w-full aspect-square max-w-[500px] md:w-[700px] md:h-[700px] flex justify-center items-center mt-12 md:mt-16">
                            ...
                            {/* SVG layer for trails and rings */}
                            <svg className="absolute w-full h-full" viewBox="0 0 700 700" fill="none" xmlns="http://www.w3.org/2000/svg">
                                ...
                                {/* NEW: Rotating dashed line trails, synchronized with icons */}
                                <circle 
                                  cx="350" cy="350" r="325" 
                                  stroke="url(#comet-gradient)" strokeWidth="2" strokeLinecap="round" strokeDasharray="15 45"
                            ...
                            {/* HTML layer for icons (sits on top of SVG) */}
                ...
                <div className="w-full flex flex-col justify-center items-center relative overflow-hidden font-poppins p-8 md:p-16 z-10 bg-gray-50">
                    <div className="relative z-10 text-center flex flex-col items-center w-full max-w-6xl mx-auto">
                        ...
                        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-8 w-full mb-12">
                            <PromiseFeatureCard icon={Clock} title="Quick Replies" description="We ensure a rapid response to all your inquiries and requests." />
                            <PromiseFeatureCard icon={Headset} title="Personalized Support" description="Get tailored support from a dedicated team that understands your needs." />
                            <PromiseFeatureCard icon={Smile} title="Friendly Experts" description="Our team of friendly professionals is ready to provide expert guidance." />
                            <PromiseFeatureCard icon={MessageSquareText} title="24/7 Assistance" description="We are here to help you around the clock, whenever you need us." />
                        </div>
                    </div>
                </div>
            </div>
